...
class FaceDetector:
    ...
    def detect(self, frame):
        ...
        if self.process_this_frame:
            ...
            # 将图像从 BGR 颜色空间（OpenCV 使用的颜色空间）转换为 RGB 颜色空间（face_recognition 使用的颜色空间）
            rgb_small_frame = np.ascontiguousarray(small_frame[:, :, ::-1])
            # 在当前帧视频中找到所有的人脸位置和人脸编码
        ...
        rects = self.detector(ray, 0)  # 检测图像中的人脸
        for i, rect in enumerate(rects):
            shape = self.predictor(ray, rect)  # 获取面部关键点
